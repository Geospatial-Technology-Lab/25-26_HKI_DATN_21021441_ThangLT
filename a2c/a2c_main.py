...
# Import environment and A2C
from environment.env_src import create_enhanced_crop_thermal_env  
from a2c.a2c import MultiAgentA2C

# Disable console logging
logging.getLogger().handlers = []
logging.getLogger().setLevel(logging.CRITICAL)
logging.getLogger('A2C_Evaluation').propagate = False
...
def setup_dual_logging():
    ...
    train_file = logging.FileHandler(
        f'logs/training_a2c_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
    )
    ...
    # Evaluation logger
    eval_logger = logging.getLogger('A2C_Evaluation')
    eval_logger.setLevel(logging.INFO)
    eval_logger.propagate = False
    
    eval_file = logging.FileHandler(
        f'logs/evaluation_a2c_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
    )
...
def plot_training_progress_a2c(rewards_history, save_path="a2c_training_plots", 
                                training_stats=None):
    """Plot A2C training progress"""
    ...
    plt.title('A2C Training Episode Rewards')
    plt.xlabel('Updates')
    plt.ylabel('Reward')
    plt.legend()
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'a2c_training_progress.png'), dpi=150, bbox_inches='tight')
    plt.close()
    print(f"A2C training progress plot saved to {os.path.join(save_path, 'a2c_training_progress.png')}")

def create_prediction_map_from_a2c(trainer, thermal_data, start_pos, weather_patches, 
                                    landcover_data, max_steps=500):
    """Create prediction map using A2C model - IMPROVED VERSION"""
    ...
    # Use first agent's network for evaluation
    model = trainer.agents[0].network
    model.eval()
    ...
    with torch.no_grad():
        for batch_start in range(0, len(positions), batch_size):
            ...
            states_tensor = torch.FloatTensor(np.array(batch_states)).to(trainer.device)
            ...
            # Process each position - USE VALUE FUNCTION AS PRIMARY SIGNAL
            for idx, (i, j) in enumerate(batch_positions):
                ...
                landcover = landcover_data[i, j]
                
                # IMPROVED FIRE DETECTION:
                # 1. High value indicates potential fire location
                # 2. Consider thermal data
                # 3. Consider landcover
                
                # Normalize value to [0, 1] range
                # Assume value range is roughly [-20, 0]
                normalized_value = (value + 20) / 20  # Higher is better
                normalized_value = np.clip(normalized_value, 0, 1)
                
                # Temperature contribution
                if local_temp > env.high_temp_threshold:
                    temp_score = 1.0
                elif local_temp > env.medium_temp_threshold:
                    temp_score = 0.7
                else:
                    temp_score = 0.3
                
                # Landcover contribution
                if landcover == 1:  # Forest
                    landcover_score = 1.0
                else:
                    landcover_score = 0.5
                
                # Combined fire probability
                # Value function is most important (60%), temp (30%), landcover (10%)
                fire_score = (0.6 * normalized_value + 
                             0.3 * temp_score + 
                             0.1 * landcover_score)
                ...
                fire_prob_map[i, j] = fire_score if fire_score > threshold else 0.0
    
    env.close()
    return V, fire_prob_map

def process_single_patch_a2c(patch_data, trainer, step, transform_affine, 
                              src_crs, patch_coords, original_shape):
    """Process single patch with A2C"""
    try:
        i = patch_data['index']
        
        V, _ = create_prediction_map_from_a2c(
            trainer,
...
def enhanced_a2c_main_with_real_data(thermal_path: str,
                                   landcover_path: str,
                                   weather_tifs: dict,
                                   ...
                                   force_retrain=False):
    """Main function for Enhanced A2C training with real data"""
    
    train_logger, eval_logger = setup_dual_logging()

    print("="*80)
    print("ENHANCED A2C TRAINING WITH REAL DATA (SYNCHRONOUS)")
    print(f"Training log: logs/training_a2c_*.log")
    print(f"Evaluation log: logs/evaluation_a2c_*.log")
    print("="*80)
    ...
    result_dirs = ["a2c_results", "a2c_models", "a2c_plots"]
    for folder in result_dirs:
        os.makedirs(folder, exist_ok=True)

    # Backup old model if exists and force_retrain
    if force_retrain and os.path.exists('a2c_models/best_a2c_model.pth'):
        backup_path = f'a2c_models/best_a2c_model_backup_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pth'
        os.rename('a2c_models/best_a2c_model.pth', backup_path)
    ...
    if model_loaded and not force_retrain:
        print("\n" + "="*80)
        print("LOADED EXISTING A2C MODEL - STARTING EVALUATION")
        print("="*80)
        
        # Verify model
        print("\n-> Verifying model functionality...")
        with torch.no_grad():
            trainer.agents[0].network.eval()
            test_state = torch.randn(10, state_size).to(trainer.device)
            test_policies, test_values = trainer.agents[0].network(test_state)
        ...
        for i, patch in enumerate(tqdm(patch_list, desc="Processing patches")):
            result = process_single_patch_a2c(
        ...
        try:
            ...
            csv_path = "a2c_results/a2c_results_summary.csv"
            df_results.to_csv(csv_path, index=False)
            print(f"[OK] Results saved to {csv_path}")

            print(f"\n{'='*60}")
            print(f"A2C RESULTS SUMMARY ({len(all_results)} patches)")
            print(f"{'='*60}")
        ...
        # TRAINING MODE
        train_logger = logging.getLogger('Training')
        train_logger.info("Starting A2C training...")
        print("\n" + "="*80)
        ...
        for update in range(1, num_updates + 1):
            ...
            if episode_rewards:
                ...
                if avg_reward > trainer.best_reward:
                    ...
                    trainer.save_model()
                    print("[BEST]", end=" ")
                    train_logger.info(f"New best A2C model saved: {avg_reward:.4f}")
            ...
            if update % 10 == 0:
                ...
                with torch.no_grad():
                    ...
                    policies, values = trainer.agents[0].network(test_state)
            ...
            if update % save_interval == 0:
                checkpoint_path = f"a2c_models/a2c_model_update_{update}.pth"
        ...
        history_path = 'a2c_models/training_history.json'
...
if __name__ == "__main__":
    ...
    try:
        ...
        if results is not None:
            print("\n" + "="*80)
            print("A2C PIPELINE COMPLETED SUCCESSFULLY!")
            print("="*80)
            print(f"Results saved to: a2c_results/a2c_results_summary.csv")
            print(f"Best model saved to: a2c_models/best_a2c_model.pth")
            print(f"Training plots saved to: a2c_plots/")
            print(f"Training logs saved to: logs/")
        else:
            print("\n[WARNING] Pipeline completed but no results generated")
        
    except Exception as e:
        print(f"\n[ERROR] Error during A2C execution: {str(e)}")
        import traceback
        traceback.print_exc()
