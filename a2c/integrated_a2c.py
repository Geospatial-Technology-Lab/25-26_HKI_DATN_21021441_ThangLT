"""
Integrated A2C Agent with CNN Observation and ICM Exploration
"""
import torch
...
class IntegratedA2CAgent:
    """A2C Agent with CNN + ICM - synchronous advantage actor-critic"""
    
    def __init__(self, num_channels=8, patch_size=11, action_size=6,
                 device='cpu', lr=1e-4, gamma=0.99, entropy_coeff=0.01,
                 value_coeff=0.5, gae_lambda=0.95, use_icm=True,
...
class IntegratedMultiAgentA2C:
    """Multi-agent A2C with CNN + ICM"""
    ...
    def __init__(self, env_factory, num_agents, num_channels=8, patch_size=11,
        ...
        self.device = device
        self.agents = [
            IntegratedA2CAgent(num_channels, patch_size, action_size, device, use_icm=use_icm, **kwargs)
            for _ in range(num_agents)
        ]
        self.envs = [env_factory() for _ in range(num_agents)]
        self.best_model_path = "a2c_models/best_integrated_a2c_model.pth"
