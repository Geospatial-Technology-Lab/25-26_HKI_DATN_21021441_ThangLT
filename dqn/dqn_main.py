...
# TODO Import fixed environment and DQN implementation
from environment.env_src import create_enhanced_crop_thermal_env  
from dqn.dqn import DQNAgent, MultiAgentDQN

# Simple logging setup - DISABLE all console output
logging.getLogger().handlers = []
logging.getLogger().setLevel(logging.CRITICAL)
logging.getLogger('DQN_Evaluation').propagate = False
...
def setup_dual_logging():
    ...
    train_file = logging.FileHandler(
        f'logs/dqn_training_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
    )
    ...
    eval_logger = logging.getLogger('DQN_Evaluation')
    eval_logger.setLevel(logging.INFO)
    eval_logger.propagate = False
    
    eval_file = logging.FileHandler(
        f'logs/dqn_evaluation_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
    )
...
def plot_dqn_training_progress(rewards_history, loss_history, epsilon_history, 
                               q_values_history=None, save_path="dqn_training_plots"):
    ...
    axes[0, 0].set_title('DQN Training Episode Rewards', fontsize=12, fontweight='bold')
    ...
    # Plot 2: Loss
    if loss_history:
        axes[0, 1].plot(loss_history, color='#E74C3C', linewidth=2)
        axes[0, 1].set_title('Training Loss (Huber)', fontsize=12, fontweight='bold')
    ...
    if q_values_history:
        ...
        axes[1, 1].plot(rewards_history, alpha=0.3, color='#2E86C1', label='Raw')
        axes[1, 1].plot(range(window-1, len(rewards_history)), moving_avg, 
                       color='#2E86C1', linewidth=2, label=f'{window}-update MA')
        axes[1, 1].set_title('Reward Moving Average', fontsize=12, fontweight='bold')
        axes[1, 1].set_xlabel('Updates')
    ...
    plt.tight_layout()
    plot_path = os.path.join(save_path, 'dqn_training_progress.png')
    plt.savefig(plot_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"DQN training progress plot saved to {plot_path}")

def create_prediction_map_from_dqn_fast(agent, thermal_data, start_pos, weather_patches, 
                                        landcover_data, max_steps=500):
    """
    Optimized version of prediction map generation for DQN
    """
    ...
    with torch.no_grad():
        for batch_start in range(0, len(positions), batch_size):
            ...
            # Get max Q-value as state value and fire prediction probability
            values = q_values.max(dim=1)[0]
            fire_q_values = q_values[:, 5]  # Q-value for action 5 (Predict Fire)
            
            # Convert Q-values to probabilities using softmax
            action_probs = torch.softmax(q_values, dim=1)
            fire_probs = action_probs[:, 5]
...
def process_single_patch_dqn_fast_clean(patch_data, agent, step, transform_affine, 
                                        src_crs, patch_coords, original_shape):
    """
    Process a single patch with DQN agent - clean logging
    """
    eval_logger = logging.getLogger('DQN_Evaluation')
    ...
    try:
        ...
        V, _ = create_prediction_map_from_dqn_fast(
            agent,
...
def process_patches_parallel_dqn_clean(patch_list, agent, step, transform_affine, 
                                       src_crs, patch_coords, original_shape, 
                                       num_workers=4):
    """
    Process all patches in parallel with clean progress display - DQN version
    """
    ...
    print("Check logs/dqn_evaluation_*.log for detailed progress\n")
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        for patch in patch_list:
            future = executor.submit(
                process_single_patch_dqn_fast_clean,
...
def check_and_fix_dqn_model_compatibility(trainer, force_retrain=False):
    """
    Check if saved DQN model is compatible with current environment dimensions
    """
    if force_retrain:
    ...
    print("Compatible DQN model loaded successfully")
    return True


def enhanced_dqn_main_with_real_data(thermal_path: str,
                                    landcover_path: str,
                                    weather_tifs: dict,
    ...
    """
    Main function for Enhanced DQN training with real data (multi-patch RL)
    Fully compatible with PPO version
    """
    ...
    print("="*80)
    print("ENHANCED DQN TRAINING WITH REAL DATA (MULTI-PATCH) - OPTIMIZED VERSION")
    print(f"Training log: logs/dqn_training_*.log")
    print(f"Evaluation log: logs/dqn_evaluation_*.log")
    print("="*80)
    ...
    result_dirs = ["dqn_results", "dqn_models", "dqn_plots"]
    ...
    # Initialize DQN trainer with correct dimensions
    trainer = MultiAgentDQN(
    ...
    model_loaded = check_and_fix_dqn_model_compatibility(trainer, force_retrain)

    if model_loaded and not force_retrain:
        print("Loaded existing DQN model. Generating predictions...")
        ...
        if use_parallel_processing:
            ...
            all_results = process_patches_parallel_dqn_clean(
            ...
            for patch in tqdm(patch_list, desc="Processing patches"):
                result = process_single_patch_dqn_fast_clean(
                    patch, agent, step, transform_affine,
                    src_crs, patch_coords, original_shape
                )
                if result['success']:
        ...
        try:
            ...
            # Save results
            csv_path = "dqn_results/dqn_results_summary.csv"
            df_results.to_csv(csv_path, index=False)
            print(f"[OK] Results saved to {csv_path}")

            print(f"\n{'='*60}")
            print(f"DQN RESULTS SUMMARY ({len(all_results)} patches processed)")
            print(f"{'='*60}")
        ...
        # Training mode
        train_logger = logging.getLogger('Training')
        train_logger.info("Starting DQN training...")
        print("\n" + "="*60)
        ...
        for update in range(1, num_updates + 1):
            ...
            if episode_rewards:
                ...
                if avg_stats:
                    loss = avg_stats.get('total_loss', {}).get('mean', 0)
                    epsilon = avg_stats.get('epsilon', {}).get('mean', 0)
                    ...
                    print(f" | Loss: {loss:.3f} | ε: {epsilon:.3f} | Q: {q_val:.2f}", end="")
                ...
                if avg_reward > trainer.best_reward:
                    ...
                    trainer.save_model()
                    print(" [BEST]", end="")
                    train_logger.info(f"New best DQN model saved: {avg_reward:.4f}")
            ...
            if update % 10 == 0:
                ...
                if epsilon_history:
                    print(f"  Current epsilon: {epsilon_history[-1]:.4f}")
                if q_values_history:
            ...
            if update % save_interval == 0:
                trainer.save_model(f"dqn_models/dqn_model_update_{update}.pth")
        
        # Final plot
        try:
            plot_dqn_training_progress(
                rewards_history, 
                loss_history,
                epsilon_history,
                q_values_history,
        ...
        training_history = {
            ...
            'loss': convert_to_serializable(loss_history),
            'epsilon': convert_to_serializable(epsilon_history),
            'q_values': convert_to_serializable(q_values_history)
        }
        
        with open('dqn_models/training_history.json', 'w') as f:
        ...
        print(f"Final epsilon: {epsilon_history[-1] if epsilon_history else 'N/A':.4f}")
        print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Compare with PPO if both results exist
        # if results is not None and os.path.exists("ppo_results/ppo_results_summary.csv"):
        #     print("\n" + "="*60)
        #     print("COMPARING DQN vs PPO PERFORMANCE")
        #     print("="*60)
            
        #     try:
        #         ppo_results = pd.read_csv("ppo_results/ppo_results_summary.csv")
                
        #         # Get average metrics (last row)
        #         dqn_avg = results[results['Patch'] == 'Average'].iloc[0]
        #         ppo_avg = ppo_results[ppo_results['Patch'] == 'Average'].iloc[0]
                
        #         print(f"\n{'Metric':<20} {'DQN':<12} {'PPO':<12} {'Winner':<10}")
        #         print("-" * 60)
                
        #         metrics_to_compare = ['AUC', 'Precision', 'Recall', 'F1', 'PR_AUC']
        #         for metric in metrics_to_compare:
        #             dqn_val = float(dqn_avg[metric])
        #             ppo_val = float(ppo_avg[metric])
        #             winner = 'DQN' if dqn_val > ppo_val else 'PPO' if ppo_val > dqn_val else 'Tie'
        #             print(f"{metric:<20} {dqn_val:<12.4f} {ppo_val:<12.4f} {winner:<10}")
                
        #         # MSE and Correlation (lower MSE is better)
        #         dqn_mse = float(dqn_avg['MSE'])
        #         ppo_mse = float(ppo_avg['MSE'])
        #         mse_winner = 'DQN' if dqn_mse < ppo_mse else 'PPO' if ppo_mse < dqn_mse else 'Tie'
        #         print(f"{'MSE':<20} {dqn_mse:<12.4f} {ppo_mse:<12.4f} {mse_winner:<10}")
                
        #         dqn_corr = float(dqn_avg['Pearson_Correlation'])
        #         ppo_corr = float(ppo_avg['Pearson_Correlation'])
        #         corr_winner = 'DQN' if dqn_corr > ppo_corr else 'PPO' if ppo_corr > dqn_corr else 'Tie'
        #         print(f"{'Correlation':<20} {dqn_corr:<12.4f} {ppo_corr:<12.4f} {corr_winner:<10}")
                
        #         print("="*60)
                
        #     except Exception as e:
        #         print(f"Could not compare with PPO results: {str(e)}")
        
    except Exception as e:
        print(f"Error during DQN training: {str(e)}")
        import traceback
        traceback.print_exc()
