...
def setup_logging():
    """Setup logging for value iteration - file only, no console"""
    ...
    logger.info("="*80)
    logger.info("VALUE ITERATION LOG FILE")
...
def run_async_value_iteration_with_enhanced_env(thermal_data: np.ndarray,
    ...
    """
    Run Optimized Asynchronous Value Iteration with EnhancedCropThermalEnv
    ...
    from value_iteration.vi import AsynchronousValueIterationOptimized
    ...
    # Create EnhancedCropThermalEnv from PPO
    ...
    # Create Optimized AsynchronousValueIteration solver with logger
    async_vi = AsynchronousValueIterationOptimized(env, gamma=0.9, theta=1e-4, logger=logger)
    
    # Run optimized value iteration with fewer iterations
    V_array, policy = async_vi.async_value_iteration_optimized(max_iterations=10000)
    
    env.close()
    
    return V_array, policy

def process_single_patch_value_iteration(patch_data, step, transform_affine, 
                                        src_crs, patch_coords, original_shape, logger=None):
    """Process a single patch using Value Iteration with EnhancedCropThermalEnv"""
    try:
        ...
        if logger:
            logger.info(f"Processing patch {i} with Value Iteration")
            logger.debug(f"  Thermal shape: {patch_data['thermal_data'].shape}")
            logger.debug(f"  Start position: {patch_data['start_pos']}")
        
        # Run Asynchronous Value Iteration with Enhanced Environment
        V_array, policy = run_async_value_iteration_with_enhanced_env(
...
def process_patches_sequential(patch_list, step, transform_affine, 
    ...
    for patch in tqdm(patch_list, desc="Processing patches", ncols=100):
        try:
            ...
            result = process_single_patch_value_iteration(
...
def process_patches_parallel_safe(patch_list, step, transform_affine, 
    ...
    if logger:
        logger.info(f"Starting parallel processing with {safe_workers} workers")
    ...
    for batch_start in range(0, len(patch_list), batch_size):
        ...
        if logger:
            logger.info(f"Processing batch {batch_start//batch_size + 1} ({len(batch)} patches)")
        
        with ProcessPoolExecutor(max_workers=safe_workers) as executor:
            futures = {
                executor.submit(
                    process_single_patch_value_iteration,
                    patch, step, transform_affine,
                    src_crs, patch_coords, original_shape,
                    None  # Don't pass logger to child processes
                ): patch['index'] 
                for patch in batch
            }
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if result['success']:
                            all_results.append(result)
                            if logger:
                                logger.info(f"Patch {patch_idx} completed")
                        else:
                            failed_patches.append(patch_idx)
                            if logger:
                                logger.warning(f"Patch {patch_idx} failed")
                    except Exception as e:
                        failed_patches.append(patch_idx)
                        if logger:
                            logger.error(f"Error for patch {patch_idx}: {str(e)}")
                    finally:
                        pbar.update(1)
    ...
    if failed_patches:
        print(f"\n[WARNING] {len(failed_patches)} patches failed")
        if logger:
            logger.warning(f"{len(failed_patches)} patches failed")
...
def value_iteration_main_with_format(thermal_path: str,
                                    landcover_path: str,
                                    weather_tifs: dict,
    ...
    """
    Main function for Value Iteration with clean console output
    Using asynchronous prioritized value iteration
    """
    
    logger = setup_logging()
    
    print("="*80)
    print("VALUE ITERATION WITH ASYNCHRONOUS PRIORITIZED METHOD")
    print(f"Log file: logs/value_iteration_*.log")
    print("="*80)
    
    # Write detailed info to log only
    logger.info("="*80)
    logger.info("VALUE ITERATION WITH ASYNCHRONOUS PRIORITIZED METHOD")
    ...
    result_dirs = ["combined_frames", "confusion_maps", "final_maps", "vi_results"]
    ...
    for i, (T_Celsius, landcover_patch, coord) in enumerate(zip(patches, landcover_patches, patch_coords)):
        ...
        if (
            ...
            invalid_count += 1
            logger.debug(f"Patch {i} skipped: max_temp={np.max(T_Celsius):.1f}, fire_pixels={np.sum(y_true_binary)}")
            continue
    ...
    try:
        ...
        # Save results
        csv_path = "vi_results/value_iteration_results_summary.csv"
        ...
        results_json = {
            ...
            'method': 'asynchronous_prioritized_value_iteration',
            'num_patches_processed': len(all_results),
            'patch_size': patch_size,
            'overlap': overlap
        }
        
        with open('vi_results/value_iteration_results.json', 'w') as f:
...
if __name__ == "__main__":
    ...
    # Run Value Iteration with safe configuration for Windows
    try:
        # Use fewer workers or sequential processing to avoid paging file error
        results = value_iteration_main_with_format(
        ...
        print(f"Error during Value Iteration: {str(e)}")
        import traceback
        traceback.print_exc()
    finally:
        print("\nDone!")