...
class VPGAgent:
    def __init__(self, state_size, action_size, device='cpu', lr=3e-4, gamma=0.99, 
                 entropy_coeff=0.01, value_coeff=0.5, use_baseline=True):
        self.device = device
        self.state_size = state_size
        ...
        self.use_baseline = use_baseline  # Whether to use value function as baseline
    ...
    def compute_returns(self, next_value=0):
        ...
        for i in reversed(range(len(normalized_rewards))):
            R = normalized_rewards[i] + self.gamma * R * (1.0 - self.dones[i])
            returns.insert(0, R)
            
        return returns
    
    def update(self, update_epochs=1):
        """Update the policy using Vanilla Policy Gradient (REINFORCE)"""
        ...
        if self.use_baseline:
            # Use value function as baseline (Actor-Critic style)
            advantages = returns - new_values.squeeze().detach()
            ...
            # Pure REINFORCE without baseline
            policy_loss = -(new_log_probs * returns).mean()
            
            # No value loss in pure REINFORCE
...
class MultiAgentVPG:
    def __init__(self, env_factory, num_agents, state_size, action_size, device='cpu', **kwargs):
        self.device = device
        self.agents = [VPGAgent(state_size, action_size, device, **kwargs) for _ in range(num_agents)]
        self.envs = [env_factory() for _ in range(num_agents)]
        self.best_model_path = "vpg_models/best_vpg_model.pth"
    ...
    def collect_experience(self, steps_per_update=1000):
        ...
        for agent, env in zip(self.agents, self.envs):
            ...
            # Ensure at least one complete episode
